...
class Dropdown extends React.Component {
  ...
  render() {
    ...
    const { testID, button, style, children, data } = this.props;

    return (
      <View ref={this._setContainerRef} style={{ flex: 1 }} collapsable={false} testID={testID}>
        {button}
        ...
        >
          <TouchableWithoutFeedback onPress={this._hide} accessible={false}>
            <View {...this.props} style={[StyleSheet.absoluteFillObject, { top: 10, left: 0 }]}>
              <Animated.View
              ...
              >
                ...
                >
                  {children || data && data.length ? data.map((e, i) => {
                    return (
                      <View>
                        <DropdownItem onPress={this.onSelected(e)} key={i} >{e.value}</DropdownItem>
                        <DropdownDivider />
                      </View>
                    )
                  }) : null}
